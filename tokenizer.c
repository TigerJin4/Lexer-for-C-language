  ...
  if (buffer[size_read] == '+') {  // + and ++
    ...
    /* FIXME IM NOT CORRECT. */
        ...
        if (size_read + 2 == size) {
 	      ...
 	      if (isprint(segment[0]) && buffer[size_read + 2] == '\'' && replace_escape_in_character(segment) == -1) {
 	          t = create_token(filename);
            t->linenum = *linenum;
            ...
            size_read++;
 	          ...
 	          size_read += 2;
 	      } else if (size_read + 3 < size && buffer[size_read + 3] == '\'' && replace_escape_in_character(segment) != -1) {
            t = create_token(filename);
            t->linenum = *linenum;
  	        t->type = TOKEN_CHARACTER;
            size_read++;
 	          t->data.character = replace_escape_in_character(segment);
 	          size_read += 3;
        } else {
            int total =
